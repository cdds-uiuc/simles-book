...
for j in range(4):
    ...
    ax.plot(t,y_weather,'-',label='weather')
    ax.plot(t,0.01*t-np.mean(0.01*t),'k-',label='obs trend')
    ax.plot(t,yhat,label='weather trend, T='+np.array2string(trend,precision=2)+'$^o$C/decade')
...
p_obs=1-stats.percentileofscore(T_H0,0.05)/100
p_obs


# A p-value of $p_{obs}=0.31$ implies that $31%$ of the null hypothesis distribution $T_{H_0}$ lies above the observed value $T_{obs}$. In general , **the smaller the p-value the more statistically significant the observations**.

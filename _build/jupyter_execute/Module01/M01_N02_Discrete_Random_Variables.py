...
# ## 1.1.1 Uniform discrete random variable
# 
# 
...
# Rerun this cell several times to simulate 20 realizations of a Uniform Discrete Random Variable 
# (i.e. 20 throws of the die)
X=stats.randint.rvs(low=1, high=7, size=20)
print(X)


# ## 1.1.2 Bernoulli Random Variable
...
# Generate 50 realizations of a Bernoulli random variable with $p=0.5$
X=stats.bernoulli.rvs(p=0.5, size=50)
print(X)
...
# Generate 50 realizations of a Bernoulli random variable with $p=0.9$
X=stats.bernoulli.rvs(p=0.9, size=50)
print(X)


# ## 1.1.3 Sum of two dice
# Let's take another example of a random variable. Consider rolling two fair dice
# ![image-2.png](attachment:image-2.png)
# 
# And define a random variable whose value is the sum of the two dice rolls. 
...
X=stats.randint.rvs(low=1, high=7, size=50)
Y=stats.randint.rvs(low=1, high=7, size=50)
Z=X+Y
print(Z)
...
# # 1.2 Distributions of discrete random variable
# 
...
# 
# 
# For discrete random variables these distributions functions are called **probability mass functions (pmf)**. the pmf of a random variable $X$ is: $$p(x_i)=P(X=x_i),$$ 
...
ax.plot(x, stats.bernoulli.pmf(x,p=0.5), 'ro',ms=10,
        label='pmf')
...
# 
# Note ther are several ways to compute a histogram:
# https://numpy.org/doc/stable/reference/generated/numpy.histogram.html<br>
# https://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.stats.histogram.html<br>
...
ax.hist(X_uniform,bins=bin_edges_uniform,density=False,rwidth=0.9)
...
# ### 1.2.3 Convergence of histograms to pmfs
# 
